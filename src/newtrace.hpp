...
namespace
{
	...
	using ptr_t = void*;
	using new_entry_t = struct { ptr_t ptr; std::size_t bytes; string_t file; string_t proc; int line; };
...
void* operator new (std::size_t n, const char* file, const char* func, int line)
{
	...
	try { get_ptr_map().emplace(ptr, new_entry_t{ ptr, n, file, func, line }); }
	catch(...) { }
	return ptr;
}
