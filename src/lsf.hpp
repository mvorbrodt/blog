#pragma once

#include <tuple>
#include <string>
#include <vector>
#include <utility>
#include <iterator>
#include <typeinfo>
#include <functional>
#include <unordered_map>
#include <cstddef>
#include <cstdint>



template<typename T>
constexpr decltype(auto) pack_type_transform(T&& value) { return(value); }

template<typename T>
constexpr decltype(auto) unpack_type_transform(T&& value) { return(value); }

#define MAKE_PACK_TYPE_CAST(from, to) \
	inline auto pack_type_transform(from value) { return to(value); } \
	inline auto unpack_type_transform(to value) { return from(value); }

#define MAKE_PACK_TYPE_TRANSFORM(from, to, pack_proc, unpack_proc) \
	inline decltype(auto) pack_type_transform(from&& value) { return(pack_proc(std::forward<from>(value))); } \
	inline decltype(auto) unpack_type_transform(to&& value) { return(unpack_proc(std::forward<to>(value))); }

#define PACK_TYPE_FROM_TYPE(T) std::decay_t<decltype(pack_type_transform(std::declval<T>()))>

MAKE_PACK_TYPE_CAST(std::size_t, short);

MAKE_PACK_TYPE_TRANSFORM(const char*, std::string,
	[](auto v) { return std::string(v); },
	[](auto v) { return v; });



#define PACKED_TYPE_SIZE(T) sizeof(PACK_TYPE_FROM_TYPE(T))

template<typename T>
auto packed_type_size(const T&) { return PACKED_TYPE_SIZE(T); }

#define MAKE_PACK_TYPE_SIZE(type, proc) \
	inline auto packed_type_size(const std::decay_t<type>& value) { return proc(value); }

MAKE_PACK_TYPE_SIZE(const char*, [](auto v) { return sizeof(std::strlen(v)) + std::strlen(v); });
MAKE_PACK_TYPE_SIZE(std::string, [](auto v) { return sizeof(v.length()) + v.length(); });



using buffer_t = std::vector<std::byte>;
using buffer_ptr_t = buffer_t::pointer;
using buffer_input_t = buffer_t::const_iterator;
using buffer_output_t = std::back_insert_iterator<buffer_t>;

struct buffer_walker_t
{
	buffer_walker_t(buffer_input_t& it, std::ptrdiff_t dist) : m_it{ it }, m_dist{ dist } {}
	~buffer_walker_t() { m_it += m_dist; }

private:
	buffer_input_t& m_it;
	std::ptrdiff_t m_dist;
};



template<typename T>
void pack_bytes(buffer_output_t& it, const T* ptr, std::size_t count)
{
	std::copy(buffer_ptr_t(ptr), buffer_ptr_t(ptr) + count, it);
}

template<typename T>
void pack_value(buffer_output_t& it, const T& value)
{
	pack_bytes(it, &value, sizeof(value));
}

template<typename T>
void pack_type(buffer_output_t& it, const T& value)
{
	pack_value(it, value);
}

inline void pack_type(buffer_output_t& it, const std::string& value)
{
	pack_type(it, value.length());
	pack_bytes(it, value.data(), value.length());
}

template<typename T>
void pack_it(buffer_output_t& it, T&& value)
{
	pack_type(it, pack_type_transform(std::forward<T>(value)));
}

template<typename... T>
auto packed_size(T&&... value)
{
	auto buffer_size = ((packed_type_size(std::forward<T>(value))) + ...);
	return buffer_size;
}

template<typename... T>
auto pack(T&&... value)
{
	auto buffer = buffer_t();
	auto it = buffer_output_t(buffer);
	buffer.reserve(packed_size(std::forward<T>(value)...));
	(pack_it(it, std::forward<T>(value)),...);
	return buffer;
}



template<typename T>
decltype(auto) value_cast(buffer_input_t& it)
{
	using value_ptr_t = const T*;
	return(*value_ptr_t(&*it));
}

template<typename T>
decltype(auto) unpack_value(buffer_input_t& it)
{
	auto walk = buffer_walker_t(it, sizeof(T));
	return(value_cast<T>(it));
}

template<typename T>
decltype(auto) unpack_type(buffer_input_t& it)
{
	return(unpack_value<T>(it));
}

template<>
decltype(auto) unpack_type<std::string>(buffer_input_t& it)
{
	auto length = unpack_value<std::string::size_type>(it);
	auto walk = buffer_walker_t(it, length);
	return(std::string(std::string::const_pointer(&*it), length));
}

template<typename T>
decltype(auto) unpack_it(buffer_input_t& it)
{
	using PT = PACK_TYPE_FROM_TYPE(T);
	return(unpack_type_transform(unpack_type<PT>(it)));
}

template<typename... T>
auto unpack(const buffer_t& buffer)
{
	auto it = std::begin(buffer);
	return std::tuple<T...>((unpack_it<T>(it))...);
}
